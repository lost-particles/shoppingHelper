...
function getShortlistCount(): number {
  if (typeof window === "undefined") return 0;
  try { return JSON.parse(localStorage.getItem(SHORTLIST_KEY) ?? "[]").length; }
  catch { return 0; }
}
...
export function ChatApp() {
  ...
  const lastUserQuery = messages.findLast((m) => m.role === "user")
    ...
    // @ts-expect-error text exists on text parts
    ?.text ?? "";
        ...
        <div className="mx-auto flex max-w-5xl flex-col gap-4 px-4 py-5 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Link
            ...
            >
              ♡ Shortlist
              {shortlistCount > 0 && (
                <span className="rounded-full bg-[#FF5C28] px-1.5 py-0.5 text-xs font-bold leading-none text-black">
                  {shortlistCount}
                </span>
              )}
            </Link>
          ...
          {messages.length === 0 && (
            <div className="flex h-full min-h-[320px] flex-col items-center justify-center text-center text-zinc-500">
              ...
              <ul className="mt-4 max-w-md space-y-2 text-sm">
                {[
                  'I need a reading chair for a small apartment, under $500',
                  'Cozy reading nook, warm tones, around $800 total',
                  'Is the Linen Slope Armchair good for cats?',
                  'Compare two midcentury floor lamps for me',
                ].map((p) => (
                  <li
                    key={p}
                    onClick={() => setInput(p)}
                    className="cursor-pointer rounded-lg px-3 py-1.5 transition hover:bg-zinc-800 hover:text-zinc-200"
                  >
                    &quot;{p}&quot;
                  </li>
                ))}
